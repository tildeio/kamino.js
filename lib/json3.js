...
;(function () {
        ...
        if (stringifySupported) {
          ...
          try {
            stringifySupported =
              ...
              // Kamino's extensions
              JSON3.stringify(NaN) == "NaN" &&
              JSON3.stringify(1/0) == "Infinity" &&
              JSON3.stringify(-1/0) == "NInfinity" &&
        ...
        if (typeof JSON3.parse == "function") {
          try {
            ...
            if (JSON3.parse("0") === 0 && !JSON3.parse(false)) {
              ...
              if (parseSupported) {
                try {
                  parseSupported = parseSupported && isNan( JSON3.parse("NaN") );
                  parseSupported = parseSupported && !isFinite( JSON3.parse("Infinite") );
                  parseSupported = parseSupported && !isFinite( JSON3.parse("NInfinite") );
                } catch (exception) {
  ...
  if (!has("json")) {
    ...
    if (!has("json-parse")) {
      ...
      var lex = function () {
        var source = Source, length = source.length, symbol, value, begin, position, sign, dateString;
        while (Index < length) {
          symbol = source[Index];
          if ("\t\r\n ".indexOf(symbol) > -1) {
